...
def SetupGemini(api_key: str):
    return genai.Client(api_key=api_key)

def FormatCarData(make: str, model: str, year: int, mileage: float, condition: str) -> str:
...
async def GetGeminiPrediction(model: Any, make: str, car_model: str, year: int,
                               mileage: float, condition: str) -> float:
    try:
        prompt = FormatCarData(make, car_model, year, mileage, condition)
...
def CombinePredictions(statistical_price: float, gemini_price: float,
